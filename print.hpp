//
// Created by Dysprosium on 2023/5/29.
//

#ifndef JSON_PARSER_JSON_PARSER_PRINT_HPP_
#define JSON_PARSER_JSON_PARSER_PRINT_HPP_
#include <type_traits>
#include <iostream>
#include <iomanip>
#include <string>
...
template<class T, class U>
struct _enable_if_iterable<T,
                           U,
                           std::void_t<typename std::iterator_traits<decltype(std::begin(std::declval<T const &>()))>::value_type>> {
  using type = U;
};
                                              ...
                                              typename _enable_if_iterable<T,
                                                                           typename _enable_if_c_str<T,
                                                                                                     typename _enable_if_string<
                                                                                                         T,
                                                                                                         typename _enable_if_map<
                                                                                                             T>::not_type>::not_type>::not_type>::type>::not_type> {
  static void print(T const &t) {
    std::cout << "{";
    bool once = false;
    for (auto const &v : t) {
...
#endif //JSON_PARSER_JSON_PARSER_PRINT_HPP_
